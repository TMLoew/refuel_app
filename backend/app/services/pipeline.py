...
def fetch_daily_weather_range(start_date: str, end_date: str, lat: float = LAT, lon: float = LON) -> pd.DataFrame:
    """
    Get daily max temperature + precipitation for the whole range (Europe/Zurich).
    """
    url = (
    ...
    j = r.json()["daily"]
    df = pd.DataFrame({
        "date": pd.to_datetime(j["time"]).dt.date.astype(str),
...
def build_product_mix(
    ...
    base_conversion: float = 0.35,  # % of visitors who buy a drink (tune later)
) -> str:
